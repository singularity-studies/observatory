...
Permitted draft statuses are `candidate`, `under_review`, `eligible`, `ineligible`, and `withdrawn`. Any future `eligible` status must resolve to a complete eligibility decision under the versioned rules in `PANEL.md`; registry status alone cannot establish eligibility. The registry remains empty.
